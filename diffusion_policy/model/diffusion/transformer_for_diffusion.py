...
class TransformerForDiffusion_(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
        ...
        self.ln_f = nn.LayerNorm(n_emb)
        #self.head = nn.Linear(n_emb, output_dim)
        self.head = KAN(in_features=n_emb, out_features=output_dim)
...
class  TransformerForDiffusionK(ModuleAttrMixin):
    def __init__(self,
            input_dim: int,
            output_dim: int,
            horizon: int,
        ...
        # input embedding stem
        #self.input_emb = nn.Linear(input_dim, n_emb)
        self.input_emb = SKAN(input_dim, n_emb)
        self.pos_emb = nn.Parameter(torch.zeros(1, T, n_emb))
        self.drop = nn.Dropout(p_drop_emb)
        ...
        if obs_as_cond:
            #self.cond_obs_emb = nn.Linear(cond_dim, n_emb)
            self.cond_obs_emb = SKAN(cond_dim, n_emb)
